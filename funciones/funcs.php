<?php
	...
	function emailExiste($email)
	{
		...
		$stmt = $mysqli->prepare("SELECT Rut FROM usuarios WHERE Email = ? LIMIT 1");
	...
	function enviarEmail($email, $nombre, $asunto, $cuerpo){
		...
		$mail->Host = 'mail.stoneagebarberia.cl'; //Modificar
		$mail->Port = 587; //Modificar
		
		$mail->Username = 'contacto@stoneagebarberia.cl'; //Modificar
		$mail->Password = 'stoneage2019'; //Modificar
		
		$mail->setFrom('contacto@stoneagebarberia.cl', 'Stoneage Barberia'); //Modificar
	...
	function generaTokenPass($user_id)
	{
		...
		$stmt = $mysqli->prepare("UPDATE usuarios SET token_password=?, password_request=1 WHERE Rut = ?");
	...
	function verificaTokenPass($user_id, $token){
		...
		$stmt = $mysqli->prepare("SELECT activacion FROM usuarios WHERE Rut = ? AND token_password = ? AND password_request = 1 LIMIT 1");
	...
	function cambiaPassword($password, $user_id, $token){
		...
		$stmt = $mysqli->prepare("UPDATE usuarios SET Pass = ?, token_password='', password_request=0 WHERE Rut = ? AND token_password = ?");
